...
2. **Probe to settle anything you can't decide from reading — review is exactly where probing earns its keep.** A review without the mechanics in hand degrades into grammar-checking. Whenever a verdict (keep / cut / reword) hinges on game behavior you're not certain of — a claimed requirement or refusal (rules 7, 11), what an item actually does, whether an answer is even *correct*, or a contradiction between the hint file and the notes — **don't guess and don't wave it through: branch a `tools/play.cjs` replay per Step 3.5** (snapshot-at the puzzle's `[slug]`, vary the tail) or re-replay `--strict` to confirm an answer still wins. This is cheap, headless, and deterministic, and it is the single highest-value move in a review: fabricated refusals and stale "open puzzle" claims surface here or nowhere. A probe result (the transcript line) is quotable evidence — cite it in the report. Probe load-bearing claims; don't exhaustively re-test obviously-correct steps.
   - **Read the notes before judging a hint *wrong*, not just unverified.** A raw transcript can mislead: an ambiguous mechanic the notes have already adjudicated will look like a fabrication if you reason only from the replay. Worked example (Anchorhead's `d2-records-names`): the transcript shows some name look-ups failing as "not a name you're familiar with," which reads like a broken/false hint — but the notes record that the partial failures are the *intended* pattern the player is meant to notice. Confirm against the notes' interpretation before flagging a hint as incorrect; the transcript tells you *what happens*, the notes tell you *what it means*.
   - **The cmds file's ordering is bookkeeping, not a requirements spec — don't flag a hint as wrong for doing something at a different point than the cmds do.** A `.cmds.txt` is one *successful* sequence; authors grab items early, carry them for convenience, and drop them out of habit (rule 7). The *position* of a command in the cmds file does not mean the action must happen there. Worked example (this skill's own miss on Theatre): the carpet answer uses `SEARCH BODY` (which reveals the appointment book to read), but the cmds file does `GET CORPSE` early — and because the corpse is later thrown to the pit beast, it *looked* like the answer had to `GET CORPSE` here. It does not: the corpse can be picked up any time before the pit, `SEARCH BODY` correctly reads the book, and the pit ladder fetches the corpse at the point of need (rule 2). A two-line probe settled it (`search body` → book revealed; corpse not in inventory and not required until the pit). Before flagging a hint as incomplete/wrong *relative to the cmds*, ask whether the difference is a real **requirement** or just the walkthrough's arbitrary **timing** — and probe (Step 3.5) rather than assume.
...
**Two layers defend this, and they cover different leaks.** The app blurs any question that has a `locations` array until the player reaches a matching room (a code-level gate); the rules below govern the wording. But the gate fires only at *room* granularity and only for *unreached* rooms — the moment a player enters a room, all its questions un-blur, including puzzles they haven't touched. So the gate stops **cross-room** leaks (a puzzle in a place you've never been); it does nothing about a discovery *within* a room you're standing in. That **in-room** leak is rule 1's job, and the gate is no substitute — players routinely reach a room long before solving its puzzles (the Anchorhead cupboard was gated to the Dining Room and *still* gave away "something's behind the cupboard"). Write every question as if its room is already on the player's screen.
...
**The mantra: when in doubt, drop or reduce.** This is the default that resolves every close call in this document, and it fires at every level — a whole *question* you're not sure earns its place, a *rung* that might be filler, a *clause* after an em-dash, a *word* that pre-shapes the answer ("need", "where", "behind"). The model's instinct is always to add one more clarifying phrase; here that instinct is wrong, so when you can't decide, cut. A hint that lands slightly too sparse costs the player one tap on the next rung — cheap and self-correcting. A hint that lands slightly too generous costs them the puzzle — silent and unrecoverable. The asymmetry is the whole reason the doubt resolves one way. What we generate for a stuck player is *a little visibility* — never *clarity* — and clarity arrives only at the answer rung.
...
1. **Anchor questions to the visible symptom, never the hidden answer.** The player searches by what they can see. If the puzzle is *discovering* something exists (a hidden room, a breakable wall, a fourth pearl), the question must not name it. Real Invisiclues: "How do I get into the dungeons?" — not "Where is the trapdoor under the rug?" Bad: "How do I get into the sealed-up ticket booth?" Good: "Is there more to find in the Manager's Office?" — with the booth's existence as a mid-ladder reveal.
   ...
   - **The room-inventory anti-pattern.** A question that asks the hint to *inventory a room* — "Is there anything useful in the Boiler Room?", "What's worth taking under the stage?" — is a request to scout, not a puzzle question. It manufactures a goal ("search this place"), pre-shapes the payoff as loot, and — because a room's items almost always serve *other* puzzles — duplicates coverage that belongs in those puzzles. Don't write it. The tell that separates it from the *endorsed* dead-end form: a bad question names a **place** and asks what's **in** it; a good one names a **problem the player feels** and asks whether it's real ("The Cloakroom looks like a dead end — is there anything to it?" confirms a suspicion rather than promising loot). Two cures: if the room holds a genuine *hidden* discovery, anchor to the visible dead-end symptom and let the discovery surface mid-ladder; if it just holds items consumed elsewhere, give it no entry and fold each item into the puzzle that needs it (rule 2).
...
2. **Don't presume possession or progress in the question.** "I have three pearls — where's the green one?" leaks that pearls are collectible, that there are four, and one is green. Anchor to a single observable instead: "What's this strange eye-like pearl for?" State-scoping belongs in section order and in-ladder gates, not question text. **Worked example (the corpse, Theatre):** Bad — "I found a corpse under the carpet. Ew. Now what?" This both presumes progress ("I found") *and* names a discovery — and it sat directly below the question whose whole payoff was uncovering that corpse ("MOVE CARPET ROLLS… something unpleasant is revealed underneath"), so a browser read the answer to the puzzle above before earning it. Fix: don't give the discovery its own question at all — fold it into the discovering puzzle's ladder as a mid-rung reveal ("don't hurry past what they were lying on top of" → "search what's underneath" → answer). When a "now what do I do with X?" question would name something a sibling puzzle is supposed to reveal, the answer is almost always *merge*, not *reword*.
   - **A prerequisite is not a puzzle — give it no entry of its own.** The proactive form of the merge: before writing any question, ask *does the player feel this as a goal on its own, or does it only matter as a step toward another puzzle's goal?* A discovery or sub-step that exists only to serve another puzzle — the journal behind the cupboard exists only to open the safe — is a **rung in that puzzle's ladder, not a question.** The player never independently wants "how do I get behind the cupboard"; they want "how do I open the safe." So anchor the entry to the goal the player actually holds and fold the prerequisite in as a gated mid-rung (rule 9). **The tell:** two entries that point at each other (the cupboard's answer names the safe, the safe's hint names the cupboard) are one puzzle that's been split into two leaking halves — collapse them into the entry whose goal the player feels.
     **When you fold such an entry away, don't orphan the item's acquisition.** Deleting a scout/prerequisite entry must not lose *how* to get the thing — the non-obvious command (`CLEAN LUMP`, `EXAMINE ITEMS`, `PULL LINING`). Fold that into one of two homes: the puzzle that **consumes** the item (as answer-rung provenance — "BREAK BOTTLE, the glue from the Boiler Room chest"), or the puzzle that **brings the player to the room** (Theatre's crow puzzle leaves you in the Boiler Room, so its answer lists the glue and the coal-lump pearl you grab while there). Pick whichever the player reaches first, and confirm the acquisition survives somewhere before you delete.
...
14. **One increment per rung, then stop — say less.** Each rung delivers exactly one new thing and ends.
   - **The one-sentence heuristic (a non-answer rung longer than one sentence is a warning sign).** A single rung carries a single increment, and one increment almost always fits in one sentence. So when a non-answer rung runs to two or more sentences (or one sentence padded with a second clause after an em-dash/colon/semicolon), treat that as a *tell* that a second increment, a tail, or a leading question has crept in — go find it and cut it. It's a heuristic, not a hard limit (an occasional rung legitimately needs two short sentences), but the default is one sentence, and the burden is on the longer rung to justify itself. Worked example (Theatre's pit): rung 1 "You can't fight it and you can't slip past in the dark. It's ravenous — and that hunger is the way through, but be careful: the obvious way to use it will get you killed." packs *four* increments (can't fight, can't sneak, it's hungry, the obvious approach is fatal); trimmed to one sentence — "You can't fight it or slip past it — it's ravenous." — it carries one. When a rung is too dense to make one sentence, that's usually the signal to *split it into another rung*, not to write a longer one (the pit puzzle's poison twist earned its own rung this way). The answer rung is exempt — it hands over everything and is as long as it needs to be.
   Four ways writers over-deliver, all to be cut:
...
- `locations` (question-level): add this whenever a question is specific to one or two rooms within its section. Trace the walkthrough to identify exactly where the player would be when they'd need that hint, and use only those rooms. **This now GATES the question, not just pins it** — a question with `locations` stays locked (spoiler-safe placeholder, no title shown) until the player has visited a matching room, then latches unlocked. Three authoring consequences:
  ...
  - **Cross-cutting / meta questions must stay location-LESS** so they're always available once their section opens. Anything the player needs **preemptively** (before reaching the afflicted room) or game-wide — a light source they must carry *into* darkness, score/meta ("can I get a perfect score?") — must omit `locations`; gating it to one room would hide it exactly when needed, and if the hazard is **lethal**, a room-gated question only surfaces *after* the player has already died. **But a survivable problem that manifests *in* specific rooms and is fixable after the fact *should* be gated to those rooms** — the gate then fires exactly when the player is stuck, which is when they want it. The test: **does the player need this *before* the room, or is being in the room how they discover the problem?** Before (or lethal) → location-less; in-room-and-survivable → gate to the rooms. Worked example (Theatre's darkness): the rooms under the theatre are unlit until a power switch is thrown back in the basement; darkness there is non-lethal (commands just fail with "you can't see"), so the "it's too dark to see down here" question is **gated to the dark rooms** (it un-blurs the moment the player is stuck in one) and its answer points back to the far-away switch — whereas a grue-style "bring a light before you enter" hazard would have to be location-less. The location-less "General Questions" section is the home for the preemptive/game-wide ones; a survival question that spans several acts' rooms can also live there with section- and question-level `locations` set to the union of afflicted rooms.
...
### When the puzzle has an obvious naive solution — probe it for a trap

The canonical replay proves the *intended* path works; it says nothing about what happens when the player tries the **obvious-but-wrong** version. Right object, wrong order; the bait without the prep; the lever pulled before the safeguard — these plausible shortcuts are exactly where IF games bury deaths and softlocks, and a hint that teaches only the right path leaves the player to find the trap by dying. So for any puzzle with a tempting naive solution, **branch the tail to run the naive version** and watch what happens.

Worked example (Theatre's pit, verified): the intended path is `POUR LIQUID ON BODY` then `THROW BODY IN PIT`. Probing the naive shortcut — `THROW BODY IN PIT` *without* poisoning it — the beast eats the corpse and feigns sleep ("loud snoring coming from down the pit"), then kills you the instant you climb down ("the snoring was a cruel ruse… *You have died*"). The un-poisoned throw isn't a no-op, it's a **death trap**. That one probe turned the puzzle from "one step" into "two welded sub-puzzles joined by a fatal near-miss," and it dictated the ladder: **a discovered trap becomes the wrong-theory to rule out (rule 5) at the rung where the player would otherwise take it** — the pit's poison rung exists precisely to steer the player off the fatal shortcut. A trap with no in-game warning is also a rule-23 finding: the hint may be the player's only warning, so it must actively name the danger, not just the fix.
