    ...
    while (pos < len) {
        ...
        if (chunktype == "IFmd") {
            try {
                ...
                /* Parse metadata using DOMParser (no jQuery dependency). */
                var parser = new DOMParser();
                var xmlDoc = parser.parseFromString('<root>' + dat + '</root>', 'text/xml');
                var identels = xmlDoc.querySelectorAll('identification > *');
                for (ix=0; ix<identels.length; ix++) {
                    var key = identels[ix].tagName.toLowerCase();
                    if (!metadata[key])
                        metadata[key] = identels[ix].textContent;
                }
                var bibels = xmlDoc.querySelectorAll('bibliographic > *');
                for (ix=0; ix<bibels.length; ix++) {
                    var key = bibels[ix].tagName.toLowerCase();
                    if (!metadata[key])
                        metadata[key] = bibels[ix].textContent;
                }
            } catch(e) { /* Metadata parsing failed — non-critical */ }
        }
        if (chunktype == "Dbug") {
