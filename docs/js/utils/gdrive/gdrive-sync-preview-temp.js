 ...
 * @param {string|Array<string>} gameNames - Game name(s) to compare saves for
 * @param {string} direction - 'import' or 'export'
 * @returns {Promise<Array>} Array of save items with comparison data
 */
export async function compareSaves(gameNames, direction) {

  // Normalize gameNames to array
  const names = Array.isArray(gameNames) ? gameNames : [gameNames];
 {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (gameName) {
      ...
      if (\!names.includes(saveGameName)) {
        continue;
      }
    }
