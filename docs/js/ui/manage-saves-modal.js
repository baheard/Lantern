...
const expandedBackups = new Set(); // tracks which save keys have backups expanded
...
function openDropdown(moreBtn, save, row, cloudStatus) {
  ...
  const cloudHeader = cloudStatus ? `
    <div class="ms-dropdown-cloud-status" style="color:${cloudStatus.color}">
      <span class="material-icons">${cloudStatus.icon}</span>${cloudStatus.label}
    </div>
    ${cloudStatus.action ? `
      <button class="ms-dropdown-item ms-sync-action">
        <span class="material-icons">${cloudStatus.action.icon}</span>${cloudStatus.action.label}
      </button>
      <div class="ms-dropdown-divider"></div>
    ` : ''}
  ` : '';
  ...
  const ddW = 210, ddH = cloudStatus?.action ? 150 : 110;
